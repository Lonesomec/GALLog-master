...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser("Active Learning Task for Log Anomaly Detection(US only)")
    ...
    labeled_dataset_pooling: list[Data] = raw_dataset_train[:int(args.budget * 1/3 * data_len)]  # 百分之10的随机数据作为初始标记池子
    unlabeled_dataset_pooling = raw_dataset_train[int(args.budget * 1/3 * data_len):]  # 还需寻找百分之20的数据,总共训练集百分之30
